...
from minigrid.core.world_object import Door, Key, Wall, Lava
from .historicalobs import HistoricalObsEnv

    
class LavaDoorKeyEnv(HistoricalObsEnv):
    def __init__(
        self,
        minRoomSize: int = 5,
        maxRoomSize: int = 10,
    ...
    def _gen_grid(self, width, height):
        ...
        # Randomize the lava position
        reject_fn = lambda env, pos: pos == rejectPos
        lava = Lava()
        self.place_obj(lava, (topX, topY), (sizeX, sizeY), reject_fn = reject_fn)
